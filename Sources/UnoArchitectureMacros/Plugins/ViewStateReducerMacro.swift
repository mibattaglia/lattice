...
extension ViewStateReducerMacro: ExtensionMacro {
    public static func expansion<D: DeclGroupSyntax, T: TypeSyntaxProtocol, C: MacroExpansionContext>(
        ...
        let conformance = "UnoArchitecture.ViewStateReducer"
                ...
                .arguments
        {
            ...
            for attribute in body.attributes {
                ...
                guard !attributeName.starts(with: "ViewStateReducerBuilder"),
                    !attributeName.starts(with: "UnoArchitecture.ViewStateReducerBuilder")
                else {
                    return []
                }
            }
            ...
            let builderArguments: TokenSyntax =
                if let macroGenerics {
                    .identifier("UnoArchitecture.ViewStateReducerBuilder<\(macroGenerics)>")
                } else if genericArgs.count == 1 {
                    .identifier(
                        "UnoArchitecture.ViewStateReducerBuilder<\(genericArgs.description).DomainState, \(genericArgs.description).ViewState>"
                    )
                } else {
                    .identifier("UnoArchitecture.ViewStateReducerBuilder<\(genericArgs)>")
                }
            return [
                AttributeSyntax(
