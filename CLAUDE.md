...
## Project Snapshot
- Lattice is a Swift 6 library (MVVM + unidirectional data flow) with macros and a small CLI.
- Targets: Lattice (library), LatticeMacros (macro plugin), LatticeCLI (executable), tests in Tests/.
...
- `Sources/LatticeCLI`: CLI executable + Resources.
...
## CLI usage (`lattice`)
- Commands: `help [topic]`, `meta`, `version`, `docs [topic|topics|index|search <term>]`.
- Flags: `--json`, `--text`, `--summary`, `--compact` (auto-JSON when stdout is piped).
- Docs source: `Sources/LatticeCLI/Resources/agent-docs.md` with README fallback.
- Exit codes: 0 success, 1 not_found, 2 invalid_args, 3 runtime_error, 4 io_error.
